...
int checksumCorruption2() {

	int solution = 0, min = 10000, max = -10000, sum = 0;
	int inMatrix[16][16] = {
	...
	int rows = sizeof inMatrix / sizeof inMatrix[0];
	int cols = sizeof inMatrix[0] / sizeof(int);
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			for (int jj = 0; jj < cols; jj++) {
				if (j!=jj && inMatrix[i][j]%inMatrix[i][jj]==0) {
					sum += inMatrix[i][j] / inMatrix[i][jj];
				}
			}
		}
	}

	cout << "The checksum is: " << sum << endl;
